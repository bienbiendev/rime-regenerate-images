...
export async function regenerate(event: RequestEvent) {
  // get params
  ...
  // get the related collection local api
    ...
    if (document.mimeType.includes('image')) {
      try {
        event.locals.rime.logger.info(
          `[plugin:regenerate-images] processing ${document.filename} - ${document.id}`
        );
        // Get the image
      ...
      event.locals.rime.logger.debug(
